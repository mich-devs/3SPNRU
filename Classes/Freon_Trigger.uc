...
#exec OBJ LOAD FILE=../3SPNv3177AT/Media/Sound1.uax PACKAGE=3SPNv3177AT
    ...
    if(bTeamHeal && Toucher.Length > 0)
    {
        ...
        if(PawnOwner.Health < MostHealth)
        {
            ...
            if(AverageDistance <= 100.0)
                HealthGain = MostHealth / (ThawSpeed * 2.5) * Touchers;
            else
                HealthGain = MostHealth / (ThawSpeed * 5) * Touchers;
...
{
     Team=255
     TouchSound=Sound'3SPNv3177AT.Sounds.Touch'
     ThawSpeed=5.000000
