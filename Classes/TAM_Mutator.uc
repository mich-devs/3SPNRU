...
var config bool AdrenWarning;
var int ThisTeamNeedsRez;

var config int MaxPlayersForBoost;
var config bool bAllowStatsRecovery;
var config int NecroUseScoreAward;
...
function PostBeginPlay()
{
    ...
    foreach AllActors(class'NavigationPoint', P)
        if (P.IsA('PlayerStart') || P.IsA('PathNode') || P.IsA('JumpSpot') )
        {
            MapCenter += P.Location;
            i++;
        }
...
function int FindTeamNeedsRez()
{
    local Controller C;
    local int RedLost, BlueLost, RedAlive, BlueAlive, TeamIndex;
    local bool RedCanRez, BlueCanRez;
    ...
    if(RedCanRez && RedLost > 0 && RedAlive < BlueAlive)
        return 0;
    
    if(BlueCanRez && BlueLost > 0 && BlueAlive < RedAlive)
        return 1;
...
simulated function Tick(float DeltaTime)
{
    ...
    {
        if(AdrenWarning)
            ThisTeamNeedsRez = FindTeamNeedsRez();
        else if(ThisTeamNeedsRez != 255)
            ThisTeamNeedsRez = 255;
    }
...
static function FillPlayInfo(PlayInfo PI)
{
    ...
    PI.AddSetting("3SPN", "AdrenWarning", "Adrenaline Warning", 0, 205, "Check");
    PI.AddSetting("3SPN", "MaxPlayersForBoost", "Max.Players For Boost", 0, 211, "Text", "3;0:32");
    PI.AddSetting("3SPN", "bAllowStatsRecovery", "Stats Recovery", 0, 401, "Check");    
    PI.AddSetting("3SPN", "NecroUseScoreAward", "Score for Using Necro", 0, 351, "Text", "3;0:99");    
    PI.AddSetting("3SPN", "bUseChatIcon", "Chat Icons", 0, 402, "Check");    
}

static event string GetDescriptionText(string PropName)
{
    if(PropName == "AdrenWarning")
        return "Sends warning for players when its time to use their adrenaline.";
    if(PropName == "MaxPlayersForBoost")
        return "Disable booster combo when server reaches this number of players (for prefer necro). 0 = don't use this feature.";
    if(PropName == "bAllowStatsRecovery")
        return "Enable auto-recovery of players' score and stats when they accidentally disconnects or loses their connection.";
    if(PropName == "NecroUseScoreAward")
        return "Score for using the necro adrenaline combo (BBFF).";
    if(PropName == "bUseChatIcon")
        return "Enable using icons above players' head when they typing into chat.";
...
function InitWeapons(int AssaultAmmo,int AssaultGrenades,int BioAmmo,int ShockAmmo,int LinkAmmo,int MiniAmmo,int FlakAmmo,int RocketAmmo,int LightningAmmo)
{
	...
	if(EnableNewNet)
	{
		...
		class'NewNet_RocketProj'.default.MyDamageType = class'DamTypeRocket';
		class'NewNet_SeekingRocketProj'.default.MyDamageType = class'DamTypeRocketHoming';
		...
		class'DamTypeRocket'.default.WeaponClass = class'NewNet_RocketLauncher';
		class'DamTypeRocketHoming'.default.WeaponClass = class'NewNet_RocketLauncher';
...
function ResetWeaponsToDefaults(bool bModifyShieldGun)
{
	...
	if(EnableNewNet)
	{
		...
		class'DamTypeRocket'.default.WeaponClass = class'xWeapons.RocketLauncher';
		class'DamTypeRocketHoming'.default.WeaponClass = class'xWeapons.RocketLauncher';
...
function bool CheckReplacement(Actor Other, out byte bSuperRelevant)
{
	...
	else
	{
		if(Other.IsA('Weapon'))
		{
            ...
            {
                ShieldGun(Other).FireModeClass[0] = class'ShieldFire';
            }
...
defaultproperties
{
     ...
     NewNetWeaponClasses(0)=Class'3SPNv3177AT.NewNet_ShockRifle'
     NewNetWeaponClasses(1)=Class'3SPNv3177AT.NewNet_LinkGun'
     NewNetWeaponClasses(2)=Class'3SPNv3177AT.NewNet_MiniGun'
     NewNetWeaponClasses(3)=Class'3SPNv3177AT.NewNet_FlakCannon'
     NewNetWeaponClasses(4)=Class'3SPNv3177AT.NewNet_RocketLauncher'
     NewNetWeaponClasses(5)=Class'3SPNv3177AT.NewNet_SniperRifle'
     NewNetWeaponClasses(6)=Class'3SPNv3177AT.NewNet_BioRifle'
     NewNetWeaponClasses(7)=Class'3SPNv3177AT.NewNet_AssaultRifle'
     NewNetWeaponClasses(8)=Class'3SPNv3177AT.NewNet_ShieldGun'
     NewNetWeaponNames(0)="3SPNv3177AT.NewNet_ShockRifle"
     NewNetWeaponNames(1)="3SPNv3177AT.NewNet_LinkGun"
     NewNetWeaponNames(2)="3SPNv3177AT.NewNet_MiniGun"
     NewNetWeaponNames(3)="3SPNv3177AT.NewNet_FlakCannon"
     NewNetWeaponNames(4)="3SPNv3177AT.NewNet_RocketLauncher"
     NewNetWeaponNames(5)="3SPNv3177AT.NewNet_SniperRifle"
     NewNetWeaponNames(6)="3SPNv3177AT.NewNet_BioRifle"
     NewNetWeaponNames(7)="3SPNv3177AT.NewNet_AssaultRifle"
     NewNetWeaponNames(8)="3SPNv3177AT.NewNet_ShieldGun"
     AdrenWarning=True
     bAllowStatsRecovery=True
     NecroUseScoreAward=5
     bUseChatIcon=True
     FriendlyName="3SPN"
     Description="3SPN"
     bNetTemporary=True
